...
def random_subsample(points, m):
    ...
    return jt.gather(points, dim=2, index=choice)


def random_sample(points, n):
    # points: [m, 3]
    choice = jt.randperm(points.shape[0]).to(points.device)
    new_points = points[choice[:n], :]

    if points.shape[0] < n:
        k = n - points.size(0)
        choice = jt.randint(points.size(0), (k,))
        new_points = jt.cat([new_points, points[choice]])
    return new_points
